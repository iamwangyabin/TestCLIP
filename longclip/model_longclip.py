...
class CLIP(nn.Module):
    ...
    #     x = x[torch.arange(x.shape[0]), text.argmax(dim=-1)] @ self.text_projectio
    #     return x
    #
    #
    ...
    def encode_text(self, text):
        x = self.token_embedding(text)#.type(self.dtype)  # [batch_size, n_ctx, d_model]
        # x = x + (self.positional_embedding * self.mask1) + (self.positional_embedding_res * self.mask2)
        x = x + (self.positional_embedding.to(x.device) * self.mask1.to(x.device)).type(self.dtype).to(x.device) +\
                (self.positional_embedding_res.to(x.device) * self.mask2.to(x.device)).type(self.dtype).to(x.device)
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
        x = self.ln_final(x)#.type(self.dtype)
        # Corrected typo: self.text_projectio -> self.text_projection
    ...
    # def forward(self, image, text_long,rank):
    ...
    #     #     ) / 2
    #     return loss_itcl, 0# loss_itcs

    def forward(self, image, text_long):
        """
        Forward pass that returns image and text features.
        """
        image_features_long = self.encode_image(image)
        text_features_long = self.encode_text(text_long)
        ...
        return image_features_long, text_features_long
